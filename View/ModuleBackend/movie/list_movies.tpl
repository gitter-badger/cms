...
<form class="standart clear" method='POST'>
	{if $arrList}
		<div class="list_wrap">
			<table class="table table-condensed table-striped table-bordered">
				...
				{foreach from=$arrList item=item name=logs}
					<tr class="{cycle values='odd,even'}">
						...
						<td>{$item->year}</td>
						<td class="actions">
							<a class="btn btn-mini ajax" rel="movie/edit_movie/?id={$item->ID}" href="{$item->link_edit}">{t}Edit{/t}</a>
			...
			{include file="helpers/paginator.tpl" ajax_path='movie/list_movies'}
