<form class="ajax" action='{$link_save}' method='POST'>
	{foreach from=$arrConfigVars key=key item=arrCategory}
		...
		{foreach from=$arrCategory item=arrVar}
			...
			<div style="width:400px; float:left;margin:0 0 5px 15px;">
				...
				<div style="display: table-cell;">
					...
					{elseif $arrVar->var_type eq 'radio' && $arrVar->select_values}
						{foreach from=$arrVar->select_values key=sRadioValue item=sRadioValueTranslation}
							<label class="control-label"><input type="radio" name="var_name[{$key}][{$arrVar->var_name}]" value="{$sRadioValue}" {if $sRadioValue eq $arrVar->var_value}checked='checked'{/if}/>{$sRadioValueTranslation}</label><br />
						{/foreach}
					...
					{elseif $arrVar->var_type eq 'checkbox'}
						...
						<input type="checkbox" name="var_name[{$key}][{$arrVar->var_name}]" value="1" {if $arrVar->var_value}checked=checked{/if} /> {t code="`$key`_`$arrVar->var_name`"}{$arrVar->var_name}{/t}
					{/if}
				</div>
			</div>
			{/if}
